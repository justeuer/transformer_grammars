...
def pad_to_multiple(inp: np.ndarray, seqlen: int) -> np.ndarray:
    if len(inp.shape) >= 1:
        ...
        return np.pad(inp, [(0, padding)] + [(0, 0)] * (len(inp.shape) - 1), "constant")
    else:
        return inp
...
def compute_token_types(
    ...
    for key in ("inputs", "labels"):
        if ranges is not None:
            ...
            # much a corner case.
            inp[f"{key}_ttypes"] = ranges.token_type_from_token(inp[key], use_jax=False)
        else:
...
def chunks_generator(it, ranges, maskrules) -> Generator[Chunk, None, None]:
    ...
    for tpl in it:
        ...
        try:
            ...
            logging.warning(f"Skipping index {item_idx} due to:\n {e}")
            continue
