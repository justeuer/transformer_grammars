...
def _call_model(forward, maskrules, params, state, chunk):
    ...
    output = jax.tree_util.tree_map(functools.partial(jnp.squeeze, axis=0), output)
    return output, state
...
def main(tokenizer, checkpoint_path, input_, output, add_eos, _):
    ...
    for chunk in chunks_it:
        ...
        total_tokens += len(inputs) - 1
        # total_tokens += len([dic[l] for l in labels if dic[l].startswith("▁")])
